...
## $(date +%Y-%m-%d) - Type Confusion DoS in CSRF Token Validation
**Vulnerability:** The CSRF token validation logic in `admin.php` and `api/save_locations.php` used `hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])`. In PHP 8+, `hash_equals` strictly expects strings. If an attacker submits an array (e.g., via `csrf_token[]=1` in a POST request or JSON input), it triggers a fatal `TypeError`, resulting in a Denial of Service (DoS) for that specific request and potentially logging noisy errors.
**Learning:** Native PHP cryptographic and hashing functions have strict type requirements in modern versions. User-provided input, especially from `$_POST`, `$_GET`, or `json_decode`, should never be assumed to be a scalar string.
**Prevention:** Always explicitly cast variables to strings `(string)` when passing them into functions like `hash_equals()` to gracefully fall back to the literal string `"Array"`, which safely fails the equality check without crashing the application.
