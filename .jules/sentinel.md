...
## $(date +%Y-%m-%d) - Timing Attack Vulnerability in Token Comparisons
**Vulnerability:** Several authentication and authorization tokens (`dashboard_token`, `csrf_token`) across multiple files (such as `api/security_check.php`, `index.php`, `admin.php`) were being compared using standard string comparison operators (`!==` or `===`). This could allow an attacker to guess the token through a timing attack, as standard string comparisons return early upon the first mismatched character.
**Learning:** Security-sensitive token and secret comparisons must execute in constant time to prevent leaking information about the token's content through execution duration. Additionally, type juggling or numeric parsing of JSON strings can cause `hash_equals()` to throw fatal errors if arguments aren't strictly strings.
**Prevention:** Always use `hash_equals()` instead of standard comparison operators for tokens, and ensure both arguments are explicitly cast to strings (e.g., `hash_equals((string)$token1, (string)$token2)`). Also, verify the token exists using `!empty()` prior to comparison.
