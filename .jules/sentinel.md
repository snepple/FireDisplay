...
## 2024-05-24 - Cross-Site Scripting (XSS) via Inline json_encode in admin.php
**Vulnerability:** In `admin.php`, several PHP variables were encoded into JSON and embedded directly into inline JavaScript blocks (e.g., `const existingEvents = <?= json_encode($active_events) ?>;`) without escaping HTML characters. Because `json_encode` does not escape characters like `<`, `>`, `&`, or `'` by default, an attacker could inject a payload (e.g., in an event or chore name) such as `</script><script>alert(1)</script>`, which would prematurely close the `<script>` tag and execute the malicious payload when an admin viewed the page.
**Learning:** Embedding JSON directly into inline `<script>` tags creates an XSS vulnerability if the encoded data contains unescaped HTML characters. Even though the data is valid JSON, the HTML parser processes the script tag context before the JavaScript engine evaluates the JSON.
**Prevention:** Always use the appropriate security flags `JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP` when calling `json_encode()` if the resulting JSON string is going to be embedded directly within an HTML `<script>` block. Alternatively, pass data to the frontend securely via HTML `data-*` attributes.
