...
## 2026-04-20 - [XSS in Calendar and Announcements Display via Unsanitized Data]
**Vulnerability:** Event summaries, locations, and announcements derived from external calendar APIs and user configurations were rendered into the DOM using `.innerHTML` without proper HTML escaping in `index.php` and `current_index.php`. Specifically, within the `createMeetingEventHtml`, `renderChoresPage`, and `renderCalendarDays` functions.
**Learning:** Even data retrieved from seemingly trusted APIs (like Google Calendar feeds) or local configuration files should be treated as untrusted input when injecting into the DOM. Relying on the source's data integrity is insufficient defense against Cross-Site Scripting (XSS).
**Prevention:** Extended the usage of the `escapeHtml()` utility function to sanitize `summary`, `location`, `timeStr`, and `ann.content` variables before they are interpolated into template strings and appended to the document using `.innerHTML`.
