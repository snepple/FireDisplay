...
for(let i=0; i<100000; i++) {
    let cleanedSummary="Career 7a - 7a";
    for(const r of rolesRegexps){
        if(r.regex.test(cleanedSummary)){
            foundRole2=cleanedSummary.match(r.regex)[0].trim();
            cleanedSummary=cleanedSummary.replace(r.regex,'').trim();
            break;
        }
    }
}
console.timeEnd("RegExp global");
