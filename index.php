...
<a href=\"manual_permit.html\" id=\"add-permit-link\" target=\"_blank\" title=\"Add Manual Burn Permit\" aria-label=\"Add Manual Burn Permit\" style=\"position: absolute; top: 10px; right: 10px; z-index: 1000; opacity: 0.15; color: var(--text-color); text-decoration: none; font-size: 24px; transition: opacity 0.3s; padding: 10px; border-radius: 5px;\">➕</a>
    ...
    <script>
        ...
        // ⚡ Bolt: Cache chore calculation constants to prevent redundant array ops and string parsing per day
        const choreCache = {
            anchorTime: null,
            totalChores: null
        };
        ...
        function getChoreNumber(date) {
            ...
            if (choreCache.anchorTime === null) {
                const [year, month, day] = (appConfig.chore_anchor || "2025-07-15").split('-');
                choreCache.anchorTime = new Date(Date.UTC(year, month - 1, day, 12, 0, 0)).getTime();
            }

            if (choreCache.totalChores === null) {
                const uniqueIds = [...new Set(appConfig.chores.map(item => item.id))].sort((a,b)=>a-b);
                choreCache.totalChores = parseInt(appConfig.chore_num_indices) || uniqueIds.length || 6;
            }

            const targetTime = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(), 12, 0, 0)).getTime();
            const diffDays = Math.round((targetTime - choreCache.anchorTime) / (1000 * 60 * 60 * 24));

            let choreIndex = (((diffDays % choreCache.totalChores) + choreCache.totalChores) % choreCache.totalChores) + 1;
            return choreIndex;
        }
